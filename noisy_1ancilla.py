...
def run_depolarising_scan():
    ...
    noise_model.add_all_qubit_quantum_error(
        depolarizing_error(0.001, 1), ["x", "h", "p"]) #0.001 original
    noise_model.add_all_qubit_quantum_error(
        depolarizing_error(0.01,  2), ["cx", "crx", "crz"]) #0.01 original
...
if __name__ == "__main__":
    ...
    n_panels = 0 if prob_manila is not None else 1
    ...
    #axes[0].set_title("Without noise", fontsize=13)
    ...
    fig.suptitle(f"Rodeo Spectrum — Ising $L={NUM_QUBITS}$", fontsize=14)
    plt.tight_layout()
